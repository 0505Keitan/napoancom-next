...
// ダイナミックパス(getStaticPaths)用にスラッグだけ一覧を取得
const getAllPostsWithSlugOnlySlug = functions.runWith(runOpts).https.onRequest(async (request, response: any) => {
    ...
    const entries = await fetchGraphQL(
      ...
      `query {
          blogPostCollection(limit:${
            limit ?? TOTAL_LIMIT
          },where: { slug_exists: true }, order: sys_firstPublishedAt_DESC, preview: ${preview ? 'true' : 'false'}) {
            items {
              slug
            }
          }
