...
// ゲームから全記事を取得
const getAllPostsForGame = functions
    ...
    if (request.method == 'GET') {
      ...
      if (slug == undefined || slug == '') {
        ...
        const entries = await fetchGraphQL(
          `query {
            gameCollection(limit: 1, where: {slug: "${slug}"}, order: sys_firstPublishedAt_DESC) {
              items {
                displayName
                linkedFrom {
        ...
        try {
          const result = extracter.extractPostForListsFromGame(entries);
          if (result) {
...
export default getAllPostsForGame;
