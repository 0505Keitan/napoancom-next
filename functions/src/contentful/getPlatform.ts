...
// スラッグからプラットフォームを取得
const getPlatform = functions.region('asia-northeast1').https.onRequest(async (request, response: any) => {
  ...
  if (request.method == 'GET') {
    ...
    if (slug == undefined || slug == '') {
      ...
      const entry = await fetchGraphQL(
        `query {
          platformCollection(where: { slug: "${slug}" }, preview: ${preview ? 'true' : 'false'}, limit: 1) {
            items {
      ...
      try {
        const result = extracter.extractPlatform(entry);
        if (result) {
